...
class CoreTypeNullablePropertyCheck(logger: Logger, sourceFiles: List<SourceFile>): BaseCheck(logger, sourceFiles) {
    override fun execute(): Boolean {
        ...
        sourceFiles
            ...
            .forEach { exportedProperty ->
                if (exportedProperty.type.isCoreType() && exportedProperty.type.isNullable) {
                    hasIssue = true
                    logger.error(exportedProperty, "Registered property with godot core type cannot be nullable")
                }
            }
        return hasIssue
    }
