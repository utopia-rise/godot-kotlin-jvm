...
`godot-gradle-plugin` turns a regular JVM project into jars usable by Godot-JVM.

It is responsible for:

1. configuring the user project so the enabled JVM source languages compile against the Godot-JVM libraries
...
4. packaging the runtime jars consumed by Godot-JVM
...
- `godot.common.GODOT_JVM_VERSION` is the stable binding version, for example `1.0.0`
...
- use `godot.common.GODOT_JVM_VERSION` when you need the stable public binding version without snapshot/build metadata
