...
public open class CharacterBody3D : PhysicsBody3D() {
  ...
  /**
   * Vector pointing upwards, used to determine what is a wall and what is a floor (or a ceiling) when calling [moveAndSlide]. Defaults to [godot.Vector3.UP]. As the vector will be normalized it can't be equal to [godot.Vector3.ZERO], if you want all collisions to be reported as walls, consider using [MOTION_MODE_FLOATING] as [motionMode].
   */
  @CoreTypeLocalCopy
  ...
  /**
   * Returns the floor's collision angle at the last collision point according to [upDirection], which is [godot.Vector3.UP] by default. This value is always positive and only valid after calling [moveAndSlide] and when [isOnFloor] returns `true`.
   */
  @JvmOverloads
