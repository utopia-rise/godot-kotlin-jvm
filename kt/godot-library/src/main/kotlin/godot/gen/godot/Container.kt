...
public open class Container : Control() {
  ...
  /**
   * Implement to return a list of allowed horizontal [enum Control.SizeFlags] for child nodes. This
   ...
   * **Note:** Having no size flags is equal to having [constant Control.SIZE_SHRINK_BEGIN]. As
  ...
  /**
   * Implement to return a list of allowed vertical [enum Control.SizeFlags] for child nodes. This
   * doesn't technically prevent the usages of any other size flags, if your implementation requires
   * that. This only limits the options available to the user in the Inspector dock.
   * **Note:** Having no size flags is equal to having [constant Control.SIZE_SHRINK_BEGIN]. As
   * such, this value is always implicitly allowed.
