...
public open class WebRTCPeerConnection : RefCounted() {
   ...
   * state [constant STATE_NEW]. A dictionary of [param configuration] options can be passed to
  ...
  /**
   * Returns a new [WebRTCDataChannel] (or `null` on failure) with given [param label] and
   * optionally configured via the [param options] dictionary. This method can only be called when the
   * connection is in state [constant STATE_NEW].
   ...
   * If this functions returns [constant OK], [signal session_description_created] will be called
   * when the session is ready to be sent.
   */
   ...
   * an [enum Error] different from [constant OK] is returned).
   */
   ...
   * If [param type] is `"offer"` the peer will emit [signal session_description_created] with the
   * appropriate answer.
  ...
  /**
   * Returns the ICE [enum GatheringState] of the connection. This lets you detect, for example,
  ...
  /**
   * Returns the [enum SignalingState] on the local end of the connection while connecting or
  ...
  public enum class SignalingState(
     ...
     * [WebRTCPeerConnection] is new ([constant STATE_NEW]) or that negotiation is complete and a
     * connection has been established ([constant STATE_CONNECTED]).
     */
  ...
  public companion object {
    /**
     * Sets the [param extension_class] as the default [WebRTCPeerConnectionExtension] returned when
