...
public open class GPUParticlesCollision3D internal constructor() : VisualInstance3D() {
  /**
   * The particle rendering layers ([godot.VisualInstance3D.layers]) that will be affected by the collision shape. By default, all particles that have [godot.ParticlesMaterial.collisionEnabled] set to `true` will be affected by a collision shape.
   *
  ...
  public var cullMask: Long
    get() {
      ...
      TransferContext.icall(rawPtr, ENGINEMETHOD_ENGINECLASS_GPUPARTICLESCOLLISION3D_GET_CULL_MASK,
          LONG.ordinal)
      return TransferContext.readReturnValue(LONG, false) as Long
    }
    set(`value`) {
      TransferContext.writeArguments(LONG to value)
      TransferContext.icall(rawPtr, ENGINEMETHOD_ENGINECLASS_GPUPARTICLESCOLLISION3D_SET_CULL_MASK,
          NIL.ordinal)
    }
