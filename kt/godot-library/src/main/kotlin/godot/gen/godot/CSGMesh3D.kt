...
public open class CSGMesh3D : CSGPrimitive3D() {
   ...
   * **Note:** When using an [ArrayMesh], all vertex attributes except [constant Mesh.ARRAY_VERTEX],
   * [constant Mesh.ARRAY_NORMAL] and [constant Mesh.ARRAY_TEX_UV] are left unused. Only [constant
   * Mesh.ARRAY_VERTEX] and [constant Mesh.ARRAY_TEX_UV] will be passed to the GPU.
   * [constant Mesh.ARRAY_NORMAL] is only used to determine which faces require the use of flat
   * shading. By default, CSGMesh will ignore the mesh's vertex normals, recalculate them for each
   * vertex and use a smooth shader. If a flat shader is required for a face, ensure that all vertex
   * normals of the face are approximately equal.
   */
