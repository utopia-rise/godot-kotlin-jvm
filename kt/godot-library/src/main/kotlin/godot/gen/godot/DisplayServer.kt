...
public object DisplayServer : Object() {
  ...
  /**
   * The ID that refers to a nonexisting window. This is be returned by some [DisplayServer] methods
   * if no window matches the requested result.
   */
  ...
  public enum class VSyncMode(
     ...
     * (tearing may be visible). Framerate is unlimited (notwithstanding [Engine.maxFps]).
     */
    VSYNC_DISABLED(0),
    /**
     * Default vertical synchronization mode, the image is displayed only on vertical blanking
     * intervals (no tearing is visible). Framerate is limited by the monitor refresh rate
     * (notwithstanding [Engine.maxFps]).
     */
    VSYNC_ENABLED(1),
    /**
