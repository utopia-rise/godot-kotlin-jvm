...
class NativeCallable : NativeCoreType, Callable {
    ...
    internal constructor(ktCallable: KtCallable<*>) {
        ...
        _handle = Bridge.engine_call_constructor_kt_custom_callable(ktCallable, ktCallable.variantType.ordinal, ktCallable.hashCode())
        MemoryManager.registerNativeCoreType(this, VariantType.CALLABLE)
    }
    ...
    fun bind(vararg args: Any?): NativeCallable {
        ...
        return TransferContext.readReturnValue(VariantType.CALLABLE, false) as NativeCallable
    }
    ...
    companion object {
        operator fun <T : Object> invoke(
            target: T,
            callable: T.() -> Unit
        ) = NativeCallable(target, (callable as KCallable<*>).name.camelToSnakeCase().asStringName())
