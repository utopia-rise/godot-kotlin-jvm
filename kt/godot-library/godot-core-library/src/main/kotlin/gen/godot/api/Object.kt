...
/**
 * An advanced [Variant] type. All classes in the engine inherit from Object. Each class may define new properties, methods or signals, which are available to all inheriting classes. For example, a [Sprite2D] instance is able to call [Node.addChild] because it inherits from [Node].
 *
 ...
 * ```
 *
 * Notifications are [int] constants commonly sent and received by objects. For example, on every rendered frame, the [SceneTree] notifies nodes inside the tree with a [Node.NOTIFICATION_PROCESS]. The nodes receive it and may call [Node.Process] to update. To make use of notifications, see [notification] and [_notification].
 *
 ...
 *
 * **Note:** Unlike references to a [RefCounted], references to an object stored in a variable can become invalid without being set to `null`. To check if an object has been deleted, do *not* compare it against `null`. Instead, use [@GlobalScope.isInstanceValid]. It's also recommended to inherit from [RefCounted] for classes storing data instead of [Object].
 *
 * **Note:** The `script` is not exposed like most properties. To set or get an object's [Script] in code, use [setScript] and [getScript], respectively.
 *
 * **Note:** In a boolean context, an [Object] will evaluate to `false` if it is equal to `null` or it has been freed. Otherwise, an [Object] will always evaluate to `true`. See also [@GlobalScope.isInstanceValid].
 */
@GodotBaseType
public open class Object : KtObject() {
   ...
   */
  public final fun `set`(`property`: StringName, `value`: Any?) {
   ...
   */
  public final fun setIndexed(propertyPath: NodePath, `value`: Any?) {
   ...
   * **Note:** In C#, [propertyPath] must be in snake_case when referring to built-in Godot properties. Prefer using the names exposed in the `PropertyName` class to avoid allocating a new [StringName] on each call.
   *
   * **Note:** This method does not support actual paths to nodes in the [SceneTree], only sub-property paths. In the context of nodes, use [Node.getNodeAndResource] instead.
   */
   ...
   */
  @JvmOverloads
  public final fun notification(what: Int, reversed: Boolean = false) {
  ...
  /**
   * Returns the object's unique instance ID. This ID can be saved in [EncodedObjectAsID], and can be used to retrieve this object instance with [@GlobalScope.instanceFromId].
   *
   ...
   */
  public final fun setScript(script: Any?) {
   ...
   */
  public final fun setMeta(name: StringName, `value`: Any?) {
   ...
   * **Note:** Metadata that has a name starting with an underscore (`_`) is considered editor-only. Editor-only metadata is not displayed in the Inspector and should not be edited, although it can still be found by this method.
   */
  public final fun removeMeta(name: StringName) {
  ...
  /**
   * Adds a user-defined signal named [signal]. Optional arguments for the signal can be added as an [Array] of dictionaries, each defining a `name` [String] and a `type` [int] (see [Variant.Type]). See also [hasUserSignal] and [removeUserSignal].
   *
   * ```gdscript
   * //gdscript
   ...
   */
  @JvmOverloads
  public final fun addUserSignal(signal: String, arguments: VariantArray<Any?> = godot.core.variantArrayOf()) {
  ...
  /**
   * Returns `true` if the given user-defined [signal] name exists. Only signals added with [addUserSignal] are included. See also [removeUserSignal].
   */
  ...
  /**
   * Removes the given user signal [signal] from the object. See also [addUserSignal] and [hasUserSignal].
   */
  public final fun removeUserSignal(signal: StringName) {
    TransferContext.writeArguments(STRING_NAME to signal)
   ...
   * **Note:** In C#, [property] must be in snake_case when referring to built-in Godot properties. Prefer using the names exposed in the `PropertyName` class to avoid allocating a new [StringName] on each call.
   */
  public final fun setDeferred(`property`: StringName, `value`: Any?) {
    TransferContext.writeArguments(STRING_NAME to property, ANY to value)
   ...
   */
  public final fun getSignalConnectionList(signal: StringName): VariantArray<Dictionary<Any?, Any?>> {
    TransferContext.writeArguments(STRING_NAME to signal)
   ...
   *
   * A signal can only be connected once to the same [Callable]. If the signal is already connected, this method returns [ERR_INVALID_PARAMETER] and pushes an error message, unless the signal is connected with [CONNECT_REFERENCE_COUNTED]. To prevent this, use [isConnected] first to check for existing connections.
   *
   ...
   *
   * When calling [emitSignal] or [Signal.emit], the signal parameters can be also passed. The examples below show the relationship between these signal parameters and bound parameters.
   *
   * ```gdscript
   * //gdscript
   * func _ready():
   ...
   */
  public final fun disconnect(signal: StringName, callable: Callable) {
   ...
   */
  public final fun setBlockSignals(enable: Boolean) {
   ...
   */
  public final fun notifyPropertyListChanged() {
    TransferContext.writeArguments()
   ...
   */
  public final fun setMessageTranslation(enable: Boolean) {
    TransferContext.writeArguments(BOOL to enable)
   ...
   *
   * **Note:** This method can't be used without an [Object] instance, as it requires the [canTranslateMessages] method. To translate strings in a static context, use [TranslationServer.translate].
   */
   ...
   *
   * **Note:** This method can't be used without an [Object] instance, as it requires the [canTranslateMessages] method. To translate strings in a static context, use [TranslationServer.translatePlural].
   */
  ...
  /**
   * Returns the name of the translation domain used by [tr] and [trN]. See also [TranslationServer].
   */
   ...
   */
  public final fun setTranslationDomain(domain: StringName) {
   ...
   * **Note:** In C#, [property] must be in snake_case when referring to built-in Godot properties. Prefer using the names exposed in the `PropertyName` class to avoid allocating a new [StringName] on each call.
   */
  public final fun `set`(`property`: String, `value`: Any?): Unit = set(property.asCachedStringName(), value)
   ...
   * **Note:** In C#, [propertyPath] must be in snake_case when referring to built-in Godot properties. Prefer using the names exposed in the `PropertyName` class to avoid allocating a new [StringName] on each call.
   */
  public final fun setIndexed(propertyPath: String, `value`: Any?): Unit = setIndexed(propertyPath.asCachedNodePath(), value)
   ...
   * **Note:** This method does not support actual paths to nodes in the [SceneTree], only sub-property paths. In the context of nodes, use [Node.getNodeAndResource] instead.
   */
  public final fun getIndexed(propertyPath: String): Any? = getIndexed(propertyPath.asCachedNodePath())
   ...
   * **Note:** This method is used by the Inspector dock to display a revert icon. The object must implement [_propertyCanRevert] to customize the default value. If [_propertyCanRevert] is not implemented, this method returns `false`.
   */
  public final fun propertyCanRevert(`property`: String): Boolean = propertyCanRevert(property.asCachedStringName())
   ...
   * **Note:** This method is used by the Inspector dock to display a revert icon. The object must implement [_propertyGetRevert] to customize the default value. If [_propertyGetRevert] is not implemented, this method returns `null`.
   */
  public final fun propertyGetRevert(`property`: String): Any? = propertyGetRevert(property.asCachedStringName())
   ...
   * **Note:** Metadata that has a name starting with an underscore (`_`) is considered editor-only. Editor-only metadata is not displayed in the Inspector and should not be edited, although it can still be found by this method.
   */
  public final fun setMeta(name: String, `value`: Any?): Unit = setMeta(name.asCachedStringName(), value)
   ...
   */
  @JvmOverloads
  public final fun getMeta(name: String, default: Any? = null): Any? = getMeta(name.asCachedStringName(), default)
   ...
   * Returns `true` if the given user-defined [signal] name exists. Only signals added with [addUserSignal] are included. See also [removeUserSignal].
   */
  public final fun hasUserSignal(signal: String): Boolean = hasUserSignal(signal.asCachedStringName())

  /**
   * Removes the given user signal [signal] from the object. See also [addUserSignal] and [hasUserSignal].
   */
  public final fun removeUserSignal(signal: String): Unit = removeUserSignal(signal.asCachedStringName())
   ...
   * **Note:** In C#, [signal] must be in snake_case when referring to built-in Godot signals. Prefer using the names exposed in the `SignalName` class to avoid allocating a new [StringName] on each call.
   */
  public final fun emitSignal(signal: String, vararg args: Any?): Error = emitSignal(signal.asCachedStringName(), )
   ...
   * **Note:** In C#, [method] must be in snake_case when referring to built-in Godot methods. Prefer using the names exposed in the `MethodName` class to avoid allocating a new [StringName] on each call.
   */
  public final fun call(method: String, vararg args: Any?): Any? = call(method.asCachedStringName(), )
   ...
   * ```
   */
  public final fun callDeferred(method: String, vararg args: Any?): Any? = callDeferred(method.asCachedStringName(), )
   ...
   * **Note:** In C#, [property] must be in snake_case when referring to built-in Godot properties. Prefer using the names exposed in the `PropertyName` class to avoid allocating a new [StringName] on each call.
   */
  public final fun setDeferred(`property`: String, `value`: Any?): Unit = setDeferred(property.asCachedStringName(), value)
   ...
   * **Note:** In C#, [method] must be in snake_case when referring to built-in Godot methods. Prefer using the names exposed in the `MethodName` class to avoid allocating a new [StringName] on each call.
   */
  public final fun callv(method: String, argArray: VariantArray<Any?>): Any? = callv(method.asCachedStringName(), argArray)
   ...
   * **Note:** In C#, [method] must be in snake_case when referring to built-in Godot methods. Prefer using the names exposed in the `MethodName` class to avoid allocating a new [StringName] on each call.
   */
  public final fun getMethodArgumentCount(method: String): Int = getMethodArgumentCount(method.asCachedStringName())
   ...
   * - `flags` is a combination of [ConnectFlags].
   */
  public final fun getSignalConnectionList(signal: String): VariantArray<Dictionary<Any?, Any?>> = getSignalConnectionList(signal.asCachedStringName())
   ...
   * Disconnects a [signal] by name from a given [callable]. If the connection does not exist, generates an error. Use [isConnected] to make sure that the connection exists.
   */
  public final fun disconnect(signal: String, callable: Callable): Unit = disconnect(signal.asCachedStringName(), callable)
   ...
   * **Note:** In C#, [signal] must be in snake_case when referring to built-in Godot signals. Prefer using the names exposed in the `SignalName` class to avoid allocating a new [StringName] on each call.
   */
  public final fun isConnected(signal: String, callable: Callable): Boolean = isConnected(signal.asCachedStringName(), callable)
   ...
   * **Note:** In C#, [signal] must be in snake_case when referring to built-in Godot methods. Prefer using the names exposed in the `SignalName` class to avoid allocating a new [StringName] on each call.
   */
  public final fun hasConnections(signal: String): Boolean = hasConnections(signal.asCachedStringName())
   ...
   * **Note:** This method can't be used without an [Object] instance, as it requires the [canTranslateMessages] method. To translate strings in a static context, use [TranslationServer.translate].
   */
  public final fun tr(message: String, context: String): String = tr(message.asCachedStringName(), context.asCachedStringName())
  ...
  public final fun trN(
    ...
    n: Int,
    context: String,
  ): String = trN(message.asCachedStringName(), pluralMessage.asCachedStringName(), n, context.asCachedStringName())

  /**
   * Sets the name of the translation domain used by [tr] and [trN]. See also [TranslationServer].
   */
  public final fun setTranslationDomain(domain: String): Unit = setTranslationDomain(domain.asCachedStringName())
  ...
  public object MethodBindings {
    internal val getClassPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_class", 201_670_096)

    internal val isClassPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "is_class", 3_927_539_163)

    internal val setPtr: VoidPtr = TypeManager.getMethodBindPtr("Object", "set", 3_776_071_444)

    internal val getPtr: VoidPtr = TypeManager.getMethodBindPtr("Object", "get", 2_760_726_917)

    internal val setIndexedPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "set_indexed", 3_500_910_842)

    internal val getIndexedPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_indexed", 4_006_125_091)

    internal val getPropertyListPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_property_list", 3_995_934_104)

    internal val getMethodListPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_method_list", 3_995_934_104)

    internal val propertyCanRevertPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "property_can_revert", 2_619_796_661)

    internal val propertyGetRevertPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "property_get_revert", 2_760_726_917)

    internal val notificationPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "notification", 4_023_243_586)

    internal val toStringPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "to_string", 2_841_200_299)

    internal val getInstanceIdPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_instance_id", 3_905_245_786)

    internal val setScriptPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "set_script", 1_114_965_689)

    internal val getScriptPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_script", 1_214_101_251)

    internal val setMetaPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "set_meta", 3_776_071_444)

    internal val removeMetaPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "remove_meta", 3_304_788_590)

    internal val getMetaPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_meta", 3_990_617_847)

    internal val hasMetaPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "has_meta", 2_619_796_661)

    internal val getMetaListPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_meta_list", 3_995_934_104)

    internal val addUserSignalPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "add_user_signal", 85_656_714)

    internal val hasUserSignalPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "has_user_signal", 2_619_796_661)

    internal val removeUserSignalPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "remove_user_signal", 3_304_788_590)

    internal val emitSignalPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "emit_signal", 4_047_867_050)

    internal val callPtr: VoidPtr = TypeManager.getMethodBindPtr("Object", "call", 3_400_424_181)

    internal val callDeferredPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "call_deferred", 3_400_424_181)

    internal val setDeferredPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "set_deferred", 3_776_071_444)

    internal val callvPtr: VoidPtr = TypeManager.getMethodBindPtr("Object", "callv", 1_260_104_456)

    internal val hasMethodPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "has_method", 2_619_796_661)

    internal val getMethodArgumentCountPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_method_argument_count", 2_458_036_349)

    internal val hasSignalPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "has_signal", 2_619_796_661)

    internal val getSignalListPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_signal_list", 3_995_934_104)

    internal val getSignalConnectionListPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_signal_connection_list", 3_147_814_860)

    internal val getIncomingConnectionsPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_incoming_connections", 3_995_934_104)

    internal val connectPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "connect", 1_518_946_055)

    internal val disconnectPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "disconnect", 1_874_754_934)

    internal val isConnectedPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "is_connected", 768_136_979)

    internal val hasConnectionsPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "has_connections", 2_619_796_661)

    internal val setBlockSignalsPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "set_block_signals", 2_586_408_642)

    internal val isBlockingSignalsPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "is_blocking_signals", 36_873_697)

    internal val notifyPropertyListChangedPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "notify_property_list_changed", 3_218_959_716)

    internal val setMessageTranslationPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "set_message_translation", 2_586_408_642)

    internal val canTranslateMessagesPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "can_translate_messages", 36_873_697)

    internal val trPtr: VoidPtr = TypeManager.getMethodBindPtr("Object", "tr", 1_195_764_410)

    internal val trNPtr: VoidPtr = TypeManager.getMethodBindPtr("Object", "tr_n", 162_698_058)

    internal val getTranslationDomainPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "get_translation_domain", 2_002_593_661)

    internal val setTranslationDomainPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "set_translation_domain", 3_304_788_590)

    internal val isQueuedForDeletionPtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "is_queued_for_deletion", 36_873_697)

    internal val cancelFreePtr: VoidPtr =
        TypeManager.getMethodBindPtr("Object", "cancel_free", 3_218_959_716)
  }
}
