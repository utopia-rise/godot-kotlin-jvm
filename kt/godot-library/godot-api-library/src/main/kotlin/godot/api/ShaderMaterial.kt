...
public open class ShaderMaterial : Material() {
  ...
  public final fun setShader(shader: Shader?) {
   ...
   */
  public final fun setShaderParameter(`param`: StringName, `value`: Any?) {
   ...
   * **Note:** Changes to the shader uniform will be effective on all instances using this [ShaderMaterial]. To prevent this, use per-instance uniforms with [GeometryInstance3D.setInstanceShaderParameter] or duplicate the [ShaderMaterial] resource using [Resource.duplicate]. Per-instance uniforms allow for better shader reuse and are therefore faster, so they should be preferred over duplicating the [ShaderMaterial] when possible.
   */
  public final fun setShaderParameter(`param`: String, `value`: Any?): Unit = setShaderParameter(param.asCachedStringName(), value)

  /**
   * Returns the current value set for this material of a uniform in the shader.
   */
  public final fun getShaderParameter(`param`: String): Any? = getShaderParameter(param.asCachedStringName())

  public companion object

  public object MethodBindings {
    internal val setShaderPtr: VoidPtr =
        TypeManager.getMethodBindPtr("ShaderMaterial", "set_shader", 3_341_921_675)

    internal val getShaderPtr: VoidPtr =
        TypeManager.getMethodBindPtr("ShaderMaterial", "get_shader", 2_078_273_437)

    internal val setShaderParameterPtr: VoidPtr =
        TypeManager.getMethodBindPtr("ShaderMaterial", "set_shader_parameter", 3_776_071_444)

    internal val getShaderParameterPtr: VoidPtr =
        TypeManager.getMethodBindPtr("ShaderMaterial", "get_shader_parameter", 2_760_726_917)
  }
}
