...
public open class OpenXRInteractionProfileMetadata : Object() {
   ...
   */
  public final fun registerProfileRename(oldName: String, newName: String) {
  ...
  /**
   * Registers an input/output path for the given [interactionProfile]. The profile should previously have been registered using [registerInteractionProfile]. [displayName] is the description shown to the user. [toplevelPath] specifies the bind path this input/output can be bound to (e.g. `/user/hand/left` or `/user/hand/right`). [openxrPath] is the action input/output being registered (e.g. `/user/hand/left/input/aim/pose`). [openxrExtensionName] restricts this input/output to an enabled/available extension, this doesn't need to repeat the extension on the profile but relates to overlapping extension (e.g. `XR_EXT_palm_pose` that introduces `…/input/palm_ext/pose` input paths). [actionType] defines the type of input or output provided by OpenXR.
   */
  ...
  public object MethodBindings {
    internal val registerProfileRenamePtr: VoidPtr =
        TypeManager.getMethodBindPtr("OpenXRInteractionProfileMetadata", "register_profile_rename", 3_186_203_200)

    internal val registerTopLevelPathPtr: VoidPtr =
        TypeManager.getMethodBindPtr("OpenXRInteractionProfileMetadata", "register_top_level_path", 254_767_734)

    internal val registerInteractionProfilePtr: VoidPtr =
        TypeManager.getMethodBindPtr("OpenXRInteractionProfileMetadata", "register_interaction_profile", 254_767_734)

    internal val registerIoPathPtr: VoidPtr =
        TypeManager.getMethodBindPtr("OpenXRInteractionProfileMetadata", "register_io_path", 3_443_511_926)
  }
}
