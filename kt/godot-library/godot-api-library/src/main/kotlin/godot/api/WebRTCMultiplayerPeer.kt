...
public open class WebRTCMultiplayerPeer : MultiplayerPeer() {
   ...
   */
  @JvmOverloads
  public final fun createServer(channelsConfig: VariantArray<Any?> = godot.core.variantArrayOf()): Error {
  ...
  /**
   * Initialize the multiplayer peer as a client with the given [peerId] (must be between 2 and 2147483647). In this mode, you should only call [addPeer] once and with [peerId] of `1`. This mode enables [MultiplayerPeer.isServerRelaySupported], allowing the upper [MultiplayerAPI] layer to perform peer exchange and packet relaying.
   *
   * You can optionally specify a [channelsConfig] array of [MultiplayerPeer.TransferMode] which will be used to create extra channels (WebRTC only supports one transfer mode per channel).
   */
  @JvmOverloads
  public final fun createClient(peerId: Int, channelsConfig: VariantArray<Any?> = godot.core.variantArrayOf()): Error {
   ...
   */
  @JvmOverloads
  public final fun createMesh(peerId: Int, channelsConfig: VariantArray<Any?> = godot.core.variantArrayOf()): Error {
    TransferContext.writeArguments(LONG to peerId.toLong(), ARRAY to channelsConfig)
  ...
  /**
   * Add a new peer to the mesh with the given [peerId]. The [WebRTCPeerConnection] must be in state [WebRTCPeerConnection.STATE_NEW].
   *
   * Three channels will be created for reliable, unreliable, and ordered transport. The value of [unreliableLifetime] will be passed to the `"maxPacketLifetime"` option when creating unreliable and ordered channels (see [WebRTCPeerConnection.createDataChannel]).
   */
  @JvmOverloads
   ...
   */
  public final fun removePeer(peerId: Int) {
  ...
  public object MethodBindings {
    internal val createServerPtr: VoidPtr =
        TypeManager.getMethodBindPtr("WebRTCMultiplayerPeer", "create_server", 2_865_356_025)

    internal val createClientPtr: VoidPtr =
        TypeManager.getMethodBindPtr("WebRTCMultiplayerPeer", "create_client", 2_641_732_907)

    internal val createMeshPtr: VoidPtr =
        TypeManager.getMethodBindPtr("WebRTCMultiplayerPeer", "create_mesh", 2_641_732_907)

    internal val addPeerPtr: VoidPtr =
        TypeManager.getMethodBindPtr("WebRTCMultiplayerPeer", "add_peer", 4_078_953_270)

    internal val removePeerPtr: VoidPtr =
        TypeManager.getMethodBindPtr("WebRTCMultiplayerPeer", "remove_peer", 1_286_410_249)

    internal val hasPeerPtr: VoidPtr =
        TypeManager.getMethodBindPtr("WebRTCMultiplayerPeer", "has_peer", 3_067_735_520)

    internal val getPeerPtr: VoidPtr =
        TypeManager.getMethodBindPtr("WebRTCMultiplayerPeer", "get_peer", 3_554_694_381)

    internal val getPeersPtr: VoidPtr =
        TypeManager.getMethodBindPtr("WebRTCMultiplayerPeer", "get_peers", 2_382_534_195)
  }
}
