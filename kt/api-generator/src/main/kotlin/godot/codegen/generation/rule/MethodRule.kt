...
class MethodRule : GodotApiRule<EnrichedMethodTask>(), BaseMethodeRule {
    ...
    private fun FunSpec.Builder.generateMethodCall(method: EnrichedMethod, clazz: EnrichedClass) {
        ...
        addStatement(
            ...
            MemberName(API.`object`.packageName, method.voidPtrVariableName),
...
class StringOnlyRule : GodotApiRule<EnrichedClassTask>(), BaseMethodeRule {
    ...
    private fun createStringOnlyMethod(method: EnrichedMethod, clazz: EnrichedClass, context: GenerationContext): EnrichedMethodTask? {
        ...
        if (method.arguments.none { it.type.identifier == CoreType.stringName.simpleName || it.type.identifier == CoreType.nodePath.simpleName }) {
            return null
        }
    ...
    override fun FunSpec.Builder.generateParameters(method: EnrichedMethod, context: GenerationContext) {
        method.arguments.withIndex().forEach {
            val index = it.index
            val parameterBuilder = if (it.value.type.identifier == CoreType.stringName.simpleName || it.value.type.identifier == CoreType.nodePath.simpleName) {
    ...
    override fun FunSpec.Builder.writeCode(method: EnrichedMethod, clazz: EnrichedClass) {
        val arguments = buildString {
            method.arguments.withIndex().forEach {
                ...
                if (argument.type.identifier == CoreType.stringName.simpleName) {
                    clazz.additionalImports.add(Function.asCachedStringName)
                    append(".asCachedStringName()")
                } else if (argument.type.identifier == CoreType.nodePath.simpleName) {
                    clazz.additionalImports.add(Function.asCachedNodePath)
...
class OverLoadRule : GodotApiRule<EnrichedMethodTask>() {
    override fun apply(task: EnrichedMethodTask, context: GenerationContext) = with(task.builder) {
        if (task.method.arguments.none { it.defaultValue != null && it.type.identifier != CoreType.stringName.simpleName && it.type.identifier != CoreType.nodePath.simpleName }) {
            return
        }
