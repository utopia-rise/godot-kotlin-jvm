...
LongStringQueue* LongStringQueue::instance = nullptr;
...
void LongStringQueue::send_string_to_cpp(JNIEnv* p_raw_env, jobject p_instance, jstring p_string) {
    ...
    instance->queue_string(nativeString);
}

void LongStringQueue::init(jni::Env& p_env, jni::JObject p_class_loader) {
    jni::Env env{jni::Jvm::current_env()};
    jni::JClass long_string_queue_cls = env.load_class("godot.core.LongStringQueue", p_class_loader);
    jni::FieldId long_string_queue_instance_field = long_string_queue_cls.get_static_field_id(env, "INSTANCE",
    ...
    instance = new LongStringQueue(long_string_queue_instance, p_class_loader);
}
