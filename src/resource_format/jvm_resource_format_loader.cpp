...
Ref<Resource> JvmResourceFormatLoader::load(const String& p_path, const String& p_original_path, Error* r_error, bool p_use_sub_threads, float* r_progress, CacheMode p_cache_mode) {
    ...
    const bool is_source = extension == GODOT_KOTLIN_SCRIPT_EXTENSION || extension == GODOT_JAVA_SCRIPT_EXTENSION
    ...
    if (!p_path.begins_with(GODOT_JVM_VIRTUAL_PATH_PREFIX)) {
        ...
        if (read_error != OK) { return nullptr; }
    }

    if (extension == GODOT_JVM_REGISTRATION_FILE_EXTENSION) {
        jvm_script = JvmScriptManager::get_instance()->get_or_create_script_from_name(p_path, &script_is_new);
    } else if (is_source) {
        jvm_script = JvmScriptManager::get_instance()->get_or_create_script_from_path(p_path, &script_is_new);
    } else {
    ...
    if (jvm_script.is_valid()) {
        ...
        if (!script_is_new && is_source) {
            MessageQueue::get_singleton()->push_callable(callable_mp(jvm_script.ptr(), &JvmScript::invalidate_source));
        }
#endif
    } else {
...
ResourceUID::ID JvmResourceFormatLoader::get_resource_uid(const String& p_path) const {
    ...
    if (extension == GODOT_JVM_REGISTRATION_FILE_EXTENSION) {
        ...
        const StringName fq_name = parse_source_script_info(source_code, p_path);
