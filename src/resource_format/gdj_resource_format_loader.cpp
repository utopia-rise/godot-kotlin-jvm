...
// We don't import Kotlin scripts so p_path == p_original_path
    ...
    if (!ref.is_null()) {
        // The script has been found in the TypeManager but not the resource cache. It means the files has been moved.
        // We set the resource to its new location.
    ...
    Ref<KotlinScript> placeholder = TypeManager::get_instance().create_placeholder_script(p_path);
    String source_code;
    Error load_err {kt_read_all_file_utf8(p_path, source_code)};
    placeholder->set_source_code(source_code);
