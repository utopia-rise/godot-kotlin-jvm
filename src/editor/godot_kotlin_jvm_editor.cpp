...
void reload_callback() {
    GodotKotlinJvmEditor::get_instance()->call_deferred("reload");
}
...
GodotKotlinJvmEditor::GodotKotlinJvmEditor() = default;
...
void GodotKotlinJvmEditor::_notificationv(int p_notification, bool p_reversed) {
    ...
    if (p_notification == NOTIFICATION_READY) {
        ...
        FileWatcher* asdf = new FileWatcher("res://build/libs/main.jar", reload_callback);
        asdf->start();
    }
}

void GodotKotlinJvmEditor::reload() {
    GDKotlin::get_instance().reload();
}
#endif// TOOLS_ENABLED
