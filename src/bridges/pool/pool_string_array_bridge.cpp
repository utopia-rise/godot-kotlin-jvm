...
PoolStringArrayBridge::PoolStringArrayBridge(jni::JObject p_wrapped, jni::JObject p_class_loader)
        : JavaInstanceWrapper(POOL_STRING_ARRAY_CLASS_NAME, p_wrapped, p_class_loader) {
...
uintptr_t PoolStringArrayBridge::engine_call_constructor(JNIEnv* p_raw_env, jobject p_instance, jboolean has_param) {
    return reinterpret_cast<uintptr_t>(new PoolSTRINGArray());
}
...
void PoolStringArrayBridge::engine_call_get(JNIEnv* p_raw_env, jobject p_instance, jlong p_raw_ptr) {
    ...
    TransferContext* transfer_context{GDKotlin::get_instance().transfer_context};
    transfer_context->read_args(env, args);
    Variant variant{from_uint_to_ptr<PoolStringArray>(p_raw_ptr)[args[0].operator unsigned int()]};
...
void PoolStringArrayBridge::engine_call_invert(JNIEnv* p_raw_env, jobject p_instance, jlong p_raw_ptr) {
    jni::Env env{jni::Jvm::current_env()};
    from_uint_to_ptr<PoolStringArray>(p_raw_ptr)->append(args[0].operator PoolStringArray());
}
