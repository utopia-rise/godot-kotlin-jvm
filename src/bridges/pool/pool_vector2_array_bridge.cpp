...
PoolVector2ArrayBridge::PoolVector2ArrayBridge(jni::JObject p_wrapped, jni::JObject p_class_loader)
        : JavaInstanceWrapper(POOL_VECTOR2_ARRAY_CLASS_NAME, p_wrapped, p_class_loader) {
...
uintptr_t PoolVector2ArrayBridge::engine_call_constructor(JNIEnv* p_raw_env, jobject p_instance, jboolean has_param) {
    return reinterpret_cast<uintptr_t>(new PoolVECTOR2Array());
}
...
void PoolVector2ArrayBridge::engine_call_invert(JNIEnv* p_raw_env, jobject p_instance, jlong p_raw_ptr) {
    jni::Env env{jni::Jvm::current_env()};
    from_uint_to_ptr<PoolVector2Array>(p_raw_ptr)->append(args[0].operator PoolVector2Array());
}
