...
PoolIntArrayBridge::PoolIntArrayBridge(jni::JObject p_wrapped, jni::JObject p_class_loader)
        : JavaInstanceWrapper(POOL_INT_ARRAY_BRIDGE_CLASS_NAME, p_wrapped, p_class_loader) {
    ...
    jni::JNativeMethod engine_call_empty_method{
            "engine_call_empty",
            "(J)V",
            (void*) PoolIntArrayBridge::engine_call_empty
    };
...
uintptr_t PoolIntArrayBridge::engine_call_constructor(JNIEnv* p_raw_env, jobject p_instance) {
    return reinterpret_cast<uintptr_t>(memnew(PoolIntArray));
}
...
void PoolIntArrayBridge::engine_call_empty(JNIEnv* p_raw_env, jobject p_instance, jlong p_raw_ptr) {
    jni::Env env{p_raw_env};
    TransferContext* transfer_context{GDKotlin::get_instance().transfer_context};
...
void PoolIntArrayBridge::engine_call_invert(JNIEnv* p_raw_env, jobject p_instance, jlong p_raw_ptr) {
    jni::Env env{p_raw_env};
    from_uint_to_ptr<PoolIntArray>(p_raw_ptr)->invert();
}
