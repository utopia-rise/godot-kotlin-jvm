...
void LambdaCallable::on_destroy(jni::Env& p_env) const {
    if (!has_on_destroy || !has_been_called) {
        return;
    }

    wrapped.call_void_method<false>(p_env, ON_DESTROY);
}
...
LambdaCallable::LambdaCallable(jni::Env& p_env, jni::JObject p_wrapped, Variant::Type return_type, int p_hash_code,
                               bool p_has_on_destroy) : JvmInstanceWrapper(p_env, p_wrapped) {
    has_return_value = return_type != Variant::NIL;

    hash_code = p_hash_code;
    has_on_destroy = p_has_on_destroy;
