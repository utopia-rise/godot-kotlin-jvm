...
void register_engine_types_hook(JNIEnv* p_env, jobject p_this, jobjectArray p_engine_types, jobjectArray p_singleton_names, jobjectArray p_method_names, jobjectArray p_types_of_methods) {
    ...
    for (int i = 0; i < method_names.length(env); i++) {
        ...
        GDKotlin::get_instance().engine_type_method.insert(
          i,
          ClassDB::get_method(GDKotlin::get_instance().engine_type_names[type_of_method], env.from_jstring(name))
        );
        name.delete_local_ref(env);
        type.delete_local_ref(env);
    }
...
#ifdef DEBUG_ENABLED
        LOG_VERBOSE(vformat("Loaded class %s : %s, as %s", kt_class->name, kt_class->super_class, kt_class->registered_class_name));
#endif
