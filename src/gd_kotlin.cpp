...
jni::JObject get_current_thread(jni::Env& env) {
    jni::JClass cls = env.findClass("java/lang/Thread");
...
jni::JObject to_java_url(jni::Env& env, const String& bootstrapJar) {
    jni::JClass cls = env.findClass("java/io/File");
    jni::MethodId ctor = cls.getConstructorMethodId(env, "(Ljava/lang/String;)V");
    jni::JObject path = env.newString(bootstrapJar.utf8().get_data());
...
jni::JObject create_class_loader(jni::Env& env, const String& bootstrapJar) {
    ...
    jni::JClass url_cls = env.findClass("java/net/URL");
    jni::JObjectArray urls = url_cls.newObjectArray(env, 1, {url});
    jni::JClass class_loader_cls = env.findClass("java/net/URLClassLoader");
...
void GDKotlin::init() {
    ...
    jni::JObject project_path = env.newString(project_settings->globalize_path("res://").utf8().get_data());
