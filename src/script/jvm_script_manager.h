...
class JvmScriptManager: public Object {
    ...
    HashMap<String, Ref<JvmScript>> source_path_to_script;
    ...
    void script_path_changed(JvmScript* p_script, const String& p_old_path, const String& p_new_path);
...
public:
    ...
    void create_and_update_scripts(const Vector<KtClass*>& classes);

    Ref<JvmScript> get_or_create_script_from_path(const String& p_path, bool* created);
    Ref<JvmScript> get_or_create_script_from_name(const String& p_path, bool* created);
    Ref<JvmScript> get_script_from_registered_name(const StringName& p_name) const;

    static void finalize();

#ifdef TOOLS_ENABLED
    int64_t get_last_reload() const;
#endif
};
