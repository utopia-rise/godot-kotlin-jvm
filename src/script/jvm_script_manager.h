...
class JvmScriptManager: public Object {
    ...
    Vector<Ref<WeakRef>> named_scripts;
    HashMap<StringName, Ref<WeakRef>> named_scripts_map;
    ...
    HashMap<StringName, KtClass*> name_to_kt_class;
...
public:
    ...
    KtClass* get_kt_class_from_name(const String& p_name) const;
...
Ref<SCRIPT> JvmScriptManager::get_or_create_named_script(const String& p_path, bool* created) {
    ...
    *created = false;
    ...
    Ref<SCRIPT> jvm_script = get_script_from_name(script_name);
    if (jvm_script.is_null()) {
        jvm_script.instantiate();
        jvm_script->kotlin_class = name_to_kt_class[script_name];
        *created = true;

        //TODO: Move to resource format loader
        String source;
        JvmResourceFormatLoader::read_all_file_utf8(p_path, source);
        jvm_script->set_source_code(source);

        Ref<WeakRef> weak_ref;
        weak_ref.instantiate();
        weak_ref->set_ref(jvm_script);

        named_scripts_map[script_name] = weak_ref;
        named_scripts.push_back(weak_ref);
    }
    return jvm_script;
}
