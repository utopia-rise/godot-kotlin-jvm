...
func test_method_param_has_expected_type():
	...
	var node = Node.new()
	var invocation_script = godot_tests_Invocation.new()
	assert_true(invocation_script.is_object_spatial(node3d), "Object parameter should be a Node3D.")
	assert_false(invocation_script.is_object_spatial(node), "Object parameter of type Node should not be a Node3D.")
