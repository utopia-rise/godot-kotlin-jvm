...
func test_pool_byte_array_add_delete() -> void:
	...
	assert_eq(invocation_script.pool_byte_array.size(), 0, "PackedByteArray should be empty")
	invocation_script.add_byte_to_pool_array(2)
	assert_eq(invocation_script.pool_byte_array.size(), 1, "PackedByteArray have one element")
	invocation_script.add_byte_array_to_pool_array(PackedByteArray([2, 3]))
	assert_eq(invocation_script.pool_byte_array.size(), 3, "PackedByteArray have 3 elements")
	invocation_script.delete_byte_from_pool_array(0)
	assert_eq(invocation_script.pool_byte_array.size(), 2, "PackedByteArray have 2 elements")

	# Test resize + set in particular
	invocation_script.resize_byte_pool_array(0)
	assert_eq(invocation_script.pool_byte_array.size(), 0, "PackedByteArray was emptied by resize(0)")
	...
	assert_eq(invocation_script.pool_byte_array.size(), 3, "PackedByteArray has size 3 after resize(3) and 3x set(...)")
...
func test_pool_color_array_add_delete() -> void:
	var invocation_script = godot_tests_Invocation.new()
	assert_eq(invocation_script.pool_color_array.size(), 0, "PackedColorArray should be empty")
	invocation_script.add_color_to_pool_array(Color())
	assert_eq(invocation_script.pool_color_array.size(), 1, "PackedColorArray have one element")
	...
	assert_eq(invocation_script.pool_color_array.size(), 3, "PackedColorArray have 3 elements")
	...
	assert_eq(invocation_script.pool_color_array.size(), 2, "PackedColorArray have 2 elements")

	# Test resize + set in particular
	invocation_script.resize_color_pool_array(0)
	assert_eq(invocation_script.pool_color_array.size(), 0, "PackedColorArray was emptied by resize(0)")
	...
	assert_eq(invocation_script.pool_color_array.size(), 3, "PackedColorArray has size 3 after resize(3) and 3x set(...)")
...
func test_pool_int_array_add_delete() -> void:
	var invocation_script = godot_tests_Invocation.new()
	assert_eq(invocation_script.pool_int_array.size(), 0, "PackedInt32Array should be empty")
	invocation_script.add_int_to_pool_array(2)
	assert_eq(invocation_script.pool_int_array.size(), 1, "PackedInt32Array have one element")
	...
	assert_eq(invocation_script.pool_int_array.size(), 3, "PackedInt32Array have 3 elements")
	...
	assert_eq(invocation_script.pool_int_array.size(), 2, "PackedInt32Array have 2 elements")

	# Test resize + set in particular
	invocation_script.resize_int_pool_array(0)
	assert_eq(invocation_script.pool_int_array.size(), 0, "PackedInt32Array was emptied by resize(0)")
	...
	assert_eq(invocation_script.pool_int_array.size(), 3, "PackedInt32Array has size 3 after resize(3) and 3x set(...)")
...
func test_pool_real_array_add_delete() -> void:
	var invocation_script = godot_tests_Invocation.new()
	assert_eq(invocation_script.pool_real_array.size(), 0, "PackedFloat32Array should be empty")
	invocation_script.add_real_to_pool_array(2.0)
	assert_eq(invocation_script.pool_real_array.size(), 1, "PackedFloat32Array have one element")
	assert_eq(invocation_script.get_real_from_pool_array(0), 2.0, "First element of PackedFloat32Array should be 2.0.")
	invocation_script.add_real_array_to_pool_array(PackedFloat32Array([2.0, 3.0]))
	assert_eq(invocation_script.pool_real_array.size(), 3, "PackedFloat32Array have 3 elements")
	...
	assert_eq(invocation_script.pool_real_array.size(), 2, "PackedFloat32Array have 2 elements")

	# Test resize + set in particular
	invocation_script.resize_real_pool_array(0)
	assert_eq(invocation_script.pool_real_array.size(), 0, "PackedFloat32Array was emptied by resize(0)")
	...
	assert_eq(invocation_script.pool_real_array.size(), 3, "PackedFloat32Array has size 3 after resize(3) and 3x set(...)")
...
func test_pool_string_array_add_delete() -> void:
	var invocation_script = godot_tests_Invocation.new()
	assert_eq(invocation_script.pool_string_array.size(), 0, "PackedStringArray should be empty")
	invocation_script.add_string_to_pool_array("2")
	assert_eq(invocation_script.pool_string_array.size(), 1, "PackedStringArray have one element")
	...
	assert_eq(invocation_script.pool_string_array.size(), 3, "PackedStringArray have 3 elements")
	...
	assert_eq(invocation_script.pool_string_array.size(), 2, "PackedStringArray have 2 elements")

	# Test resize + set in particular
	invocation_script.resize_string_pool_array(0)
	assert_eq(invocation_script.pool_string_array.size(), 0, "PackedStringArray was emptied by resize(0)")
	...
	assert_eq(invocation_script.pool_string_array.size(), 3, "PackedStringArray has size 3 after resize(3) and 3x set(...)")
...
func test_pool_vector2_array_add_delete() -> void:
	var invocation_script = godot_tests_Invocation.new()
	assert_eq(invocation_script.pool_vector2_array.size(), 0, "PackedVector2Array should be empty")
	invocation_script.add_vector2_to_pool_array(Vector2(2, 1))
	assert_eq(invocation_script.pool_vector2_array.size(), 1, "PackedVector2Array have one element")
	...
	assert_eq(invocation_script.pool_vector2_array.size(), 3, "PackedVector2Array have 3 elements")
	...
	assert_eq(invocation_script.pool_vector2_array.size(), 2, "PackedVector2Array have 2 elements")

	# Test resize + set in particular
	invocation_script.resize_vector2_pool_array(0)
	assert_eq(invocation_script.pool_vector2_array.size(), 0, "PackedVector2Array was emptied by resize(0)")
	...
	assert_eq(invocation_script.pool_vector2_array.size(), 3, "PackedVector2Array has size 3 after resize(3) and 3x set(...)")
...
func test_pool_vector3_array_add_delete() -> void:
	var invocation_script = godot_tests_Invocation.new()
	assert_eq(invocation_script.pool_vector3_array.size(), 0, "PackedVector3Array should be empty")
	invocation_script.add_vector3_to_pool_array(Vector3(2, 1, 1))
	assert_eq(invocation_script.pool_vector3_array.size(), 1, "PackedVector3Array have one element")
	...
	assert_eq(invocation_script.pool_vector3_array.size(), 3, "PackedVector3Array have 3 elements")
	...
	assert_eq(invocation_script.pool_vector3_array.size(), 2, "PackedVector3Array have 2 elements")

	# Test resize + set in particular
	invocation_script.resize_vector3_pool_array(0)
	assert_eq(invocation_script.pool_vector3_array.size(), 0, "PackedVector3Array was emptied by resize(0)")
	...
	assert_eq(invocation_script.pool_vector3_array.size(), 3, "PackedVector3Array has size 3 after resize(3) and 3x set(...)")
